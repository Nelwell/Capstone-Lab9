...
class TestHomePage(TestCase):
    ...
    def test_home_page_shows_empty_list_message_for_empty_database(self):
        ...
        response = self.client.get(home_page_url)  # in the background will run server and make request to server
        self.assertTemplateUsed(response, 'travel_wishlist/wishlist.html')  # looks for expected template
        self.assertContains(response, 'You have no places in your wishlist')  # look for specified message on page
...
class TestWishList(TestCase):
    ...
    def test_wishlist_not_visited_places(self):
...
class TestAssNewPlace(TestCase):
    ...
    def test_add_new_unvisited_place(self):
        ...
        response_places = response.context['places']  # what ever data the view providing to the template
...
class TestVisitPlace(TestCase):
    ...
    def test_visit_place(self):
        ...
        new_york = Place.objects.get(pk=2)  # check the db for new york using pk
    ...
    def test_non_existent_place(self):
        ...
        self.assertEqual(404, response.status_code)  # check status code is thrown when place doesn't exist
